...
def _clean_line(line: str) -> str:
    # remove things like (chorus), <chorus>, {repeat}
    line = re.sub(r"\(.*?\)|\<.*?\>|\{.*?\}", " ", line)

    # Normalize Unicode (fix curly quotes, accents, etc.)
    import unicodedata
    line = unicodedata.normalize("NFKC", line)

    # Remove ALL punctuation but keep letters, numbers, and spaces
    # \w = letters + digits + underscore
    # We remove underscore too because it's not needed for lyrics
    line = re.sub(r"[^\w\s]", " ", line)

    # Remove underscores (optional)
    line = line.replace("_", " ")
